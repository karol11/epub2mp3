...
func (enc *Encoder) Write(out io.Writer, data []int16) error {
	...
	for i := 0; i < len(data); i += samplesPerPass {
		frame := make([]int16, samplesPerPass)
		copy(frame, data[i:])
		encoded, written := enc.encodeBufferInterleaved(&frame[0])
		if err := binary.Write(out, binary.LittleEndian, encoded[:written]); err != nil {
			return err
		}
	}
	return nil
